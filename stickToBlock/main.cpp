...
std::shared_ptr<KOMO> GetKomoToTable(const rai::Configuration& config) {
  std::shared_ptr<KOMO> komo = PrepareKomo(config);

  const std::string object = "refTable";
  //task objectives:
  arr basePos = config[object.c_str()]->getPosition();
  komo.get()->addObjective({1.}, FS_position, {"stickTop"}, OT_eq, {1e2}, basePos);


  komo.get()->addObjective({1.},     FS_scalarProductZZ, {object.c_str(), "l_gripperCenter"}, OT_eq,   {1e2}, {.7});
  komo.get()->addObjective({1.},     FS_scalarProductXY, {"l_gripperCenter", object.c_str()}, OT_eq,   {1e2}, {-1.});
  komo.get()->addObjective({0., .9}, FS_distance,       {"stickTop", "table"},               OT_ineq, {1e2}, {0.05});
  komo.get()->addObjective({1.}, FS_distance,       {"stickTop", object.c_str()},               OT_eq, {1e2}, {0});
  komo.get()->addObjective({1.}, FS_qItself, {}, OT_eq, {1e2}, {}, 1);
